...
halibut = read.csv("Data/halibut.csv")

# Natural mortality (M) and fecundity (f) for each age
lh = read.csv("Data/halibut_lh.csv")

# Weight at each age during each year
wt = read.csv("Data/halibut_wt.csv")
...
#
# Data are given for each age for each year.  Year classes are along
# the diagonals.
...
hmat = as.matrix(halibut[,-c(1,ncol(halibut))])
rownames(hmat) = halibut$Year
hmat
...
# Copy the cohorts list structure to store population and fishing mortality estimates
N = lapply(cohorts, function(fred) rep(NA,length(fred)))
fmort = N

# The last year's fishing mortality for each cohort (I took these
# from the Quinn and Deriso book)
...
for(i in 1:length(fmort)) fmort[[i]] [length(fmort[[i]])] = FA[i]
...
# I've got you started here with your arguments and function name.
gulland = function(Fmort, Mmort, catch) {
  Z = Fmort + Mmort	
  last(catch) / ((Fmort/Z)*(1-exp(-Z)))
}
...
# for each cohort
N.last = mapply(gulland, catch = cohorts, Fmort = FA, Mmort = MA)

# Store in last element of each vector in the N estimates list
for(i in 1:length(N)) N[[i]] [length(N[[i]])] = N.last[i]
...
pope = function(catch, N, Fmort, M, Nret = TRUE) {
  for(i in (length(catch)-1):1) {
    N[i] = N[i+1] * exp(M) + catch[i] * exp(M/2)
    Fmort[i] = -log(N[i+1]/N[i]) - M
  }
  if(Nret) return(N) else return(Fmort)
  #return(list(N = N, Fmort = Fmort))
}

# Apply function to each cohort (that needs it! the ones with only one element don't)
# HINT: use mapply or write a loop
N[2:27] = mapply(pope, catch = cohorts[2:27], N = N[2:27], Fmort = fmort[2:27], M = MA)
fmort[2:27] = mapply(pope, catch = cohorts[2:27], N = N[2:27], Fmort = fmort[2:27], M = MA, Nret = FALSE)
...
plot(halibut$Year, rowSums(N_mat)/100000,
     xlab = "Year", ylab = "N (hundred thousands)",
     type = "l", lwd = 4)
